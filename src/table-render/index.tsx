...
const TableRender: FC<IProps> = props => {
  ...
  const toolTip = (value: ReactNode) => {
    return value
    // return (
    //   <Tooltip mouseEnterDelay={1.5} title={value} placement="topLeft">
    //     {value}
    //   </Tooltip>
    // )
  }
  ...
  const getWidgetValue = (params: {
    ...
    if (attrs?.widget) {
      ...
      if (attrs?.type === 'array') {
        const val = value?.map((item: any) => item ?? placeholder ?? attrs?.placeholder).join('/')
        return toolTip(val)
      }
      if (Component) {
        if (typeof Component === 'function') {
    ...
    const val = value ?? placeholder ?? attrs?.placeholder
    return toolTip(val)
  }
  ...
  const createInnerNode = (params: IRenderParams) => {
    ...
    if (attrs?.type === 'array') {
      value.forEach((item: any) => {
        const val = item ?? placeholder
        if (separator === 'newLine') {
          strArr.push(<span style={{ display: 'block' }}>{val}</span>)
        } else {
          strArr.push(val)
        }
      })
